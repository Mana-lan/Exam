...
workflow {
  fastqcChannel = Channel.from(params.accession)
    varfetch = prefetch(fastqcChannel) | flatten
    fastqcChannelA = fasterqdump(varfetch)
  
  fastPChannel = Channel.empty()
	
  if(params.with_fastp) {
       fastPChannel= fastPChannel.concat(fastP(fastqcChannelA))
       //the empty fastpChannel is overwite by itself concatenated with the fastqcChannelA on which fastP was applied  
	}
 
  fastqcChannelB = fastqcChannelA.concat(fastPChannel)
  fastqc(fastqcChannelB)
